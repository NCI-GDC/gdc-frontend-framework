...
class CohortComparisonLocators:
    TEXT_COHORT_CASE_COUNT_SELECTION_SCREEN = (
        lambda cohort_name: f'[data-testid="text-{cohort_name}-case-count-cohort-comparison"]'
    )
    BUTTON_SELECT_COHORT_TO_COMPARE_COHORT = (
        lambda cohort_name: f'[data-testid="button-{cohort_name}-cohort-comparison"]'
    )
    BUTTON_RUN_COHORT_COMPARISON_SELECTION_SCREEN = (
        '[data-testid="button-run-cohort-comparison"]'
    )

    LOADING_SPINNER_SURVIVAL_PLOT = (
        '[data-testid="card-analysis-survival-cohort-comparison"] circle >> nth=0'
    )

    TEXT_COHORT_CASE_COUNT_MAIN_SCREEN = (
        lambda cohort_number: f'[data-testid="text-cohort-case-count-cohort-comparison"] >> nth={cohort_number}'
    )
    ...
    BUTTON_TSV_ANALYSIS_CARD = (
        lambda analysis_card: f'[data-testid="card-{analysis_card}-cohort-comparison"] >> [data-testid="button-tsv-cohort-comparison"]'
    )
...
class CohortComparisonPage(BasePage):
    ...
    def select_cohort_to_compare_with(self, cohort_name: str):
        ...
        """
        button_select_cohort_to_compare_locator = (
            CohortComparisonLocators.BUTTON_SELECT_COHORT_TO_COMPARE_COHORT(cohort_name)
        )
    ...
    def get_cohort_case_count_on_selection_screen(self, cohort_name: str):
        ...
        """
        cohort_case_count_selection_screen_locator = (
            CohortComparisonLocators.TEXT_COHORT_CASE_COUNT_SELECTION_SCREEN(
                cohort_name
            )
        )
    ...
    def get_cohort_case_count_on_main_screen(self, cohort_number: str):
        ...
        """
        cohort_case_count_main_screen_locator = (
            CohortComparisonLocators.TEXT_COHORT_CASE_COUNT_MAIN_SCREEN(cohort_number)
        )
    ...
    def click_run_comparison_cohort_comparison_selection_screen(self):
        button_run_comparison_locator = (
            CohortComparisonLocators.BUTTON_RUN_COHORT_COMPARISON_SELECTION_SCREEN
        )
    ...
    def click_analysis_card_button_enable_disable(self, analysis_card_name: str):
        ...
        button_enable_disable_analysis_card_locator = (
            CohortComparisonLocators.BUTTON_ADD_REMOVE_ANALYSIS_CARD(analysis_card_name)
        )
    ...
    def is_analysis_card_visible_cohort_comparison(self, analysis_card_name: str):
        analysis_card_locator = CohortComparisonLocators.ANALYSIS_CARD_IDENT(
            analysis_card_name
        )
        return self.is_visible(analysis_card_locator)

    def get_case_count_on_save_cohort_button_on_analysis_card_cohort_comparison(
        self, analysis_card: str, row_name: str, cohort_number: str
    ):
        """
        ...
        save_cohort_locator = CohortComparisonLocators.BUTTON_SAVE_COHORT_ANALYSIS_CARD(
            analysis_card, row_name, cohort_number
        )
        return self.get_text(save_cohort_locator)

    def click_save_cohort_button_on_analysis_card_cohort_comparison(
        self, analysis_card: str, row_name: str, cohort_number: str
    ):
        """
    ...
    def click_download_tsv_button_on_analysis_card_cohort_comparison(
        self, analysis_card: str
    ):
        """
        Clicks TSV button from an analysis card on the cohort comparison main screen.
        """
        tsv_download_locator = CohortComparisonLocators.BUTTON_TSV_ANALYSIS_CARD(
            analysis_card
        )
