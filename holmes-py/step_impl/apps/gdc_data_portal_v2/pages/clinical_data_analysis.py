...
class ClinicalDataAnalysisPage:
    ...
    def click_on_cdave_play_button(self):
        self.driver.locator(
            ClinicalDataAnalysisLocators.CLINICAL_DATA_ANALYSIS_PLAY_BUTTON
        ).click()
        self.driver.wait_for_selector(
            ClinicalDataAnalysisLocators.PLEASE_WAIT_SPINNER, state="hidden"
        )
        self.driver.wait_for_selector(
            ClinicalDataAnalysisLocators.GROUP_TABLE("Demographic"), state="visible"
        )

    def expand_clinical_property_sections(self):
        self.driver.locator(
            ClinicalDataAnalysisLocators.GROUP_TABLE_PLUS_BUTTON("Demographic")
        ).click()
        self.driver.locator(
            ClinicalDataAnalysisLocators.GROUP_TABLE_PLUS_BUTTON("Diagnosis")
        ).click()
        self.driver.locator(
            ClinicalDataAnalysisLocators.GROUP_TABLE_PLUS_BUTTON("Treatment")
        ).click()
        self.driver.locator(
            ClinicalDataAnalysisLocators.GROUP_TABLE_PLUS_BUTTON("Exposures")
        ).click()
