...
@step(
    "Select cohort <cohort_name> for comparison on the cohort comparison selection screen"
)
...
@step(
    "Collect case count of cohort <cohort_name> for comparison on the cohort comparison selection screen"
)
...
def store_cohort_case_count_main_screen_cohort_comparison():
    ...
    cohort_case_count_s2 = (
        APP.cohort_comparison_page.get_cohort_case_count_on_main_screen("1")
    )
...
@step(
    "Verify analysis cards are visible or not visible as expected on Cohort Comparison <table>"
)
def is_analysis_card_visible_or_not_visible_as_expected(table):
    for k, v in enumerate(table):
        is_analysis_card_visible = (
            APP.cohort_comparison_page.is_analysis_card_visible_cohort_comparison(v[0])
        )
        should_analysis_card_be_visible_or_not_visible = v[1].lower()
        if should_analysis_card_be_visible_or_not_visible == "visible":
            assert (
                is_analysis_card_visible
            ), f"The analysis card {v[0]} is NOT visible when it should be"
        elif should_analysis_card_be_visible_or_not_visible == "not visible":
            assert (
                is_analysis_card_visible == False
            ), f"The analysis card {v[0]} is visible when it should NOT be"


@step(
    "Collect case counts on save cohort buttons from an analysis card on Cohort Comparison <table>"
)
def collect_case_counts_on_save_cohort_buttons_from_analysis_card_cohort_comparison(
    table,
):
    """
    ...
    """
    for k, v in enumerate(table):
        case_count = APP.cohort_comparison_page.get_case_count_on_save_cohort_button_on_analysis_card_cohort_comparison(
            v[0], v[1], v[2]
        )
