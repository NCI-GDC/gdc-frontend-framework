...
def make_cohort_builder_selections(table):
    for k, v in enumerate(table):
        is_filter_visible = APP.base_page.is_filter_card_present(v[0])
...
def verify_showing_item_text(number_of_items_text):
    ...
    showing_items_text = APP.base_page.get_showing_count_text()
...
def wait_for_data_testid_to_be_visible_on_the_page(data_testid: str):
    ...
    is_data_testid_visible = APP.base_page.wait_for_data_testid_to_be_visible(data_testid)
...
def is_text_present_on_the_page(expected_text: str):
    ...
    is_text_present = APP.base_page.is_text_present(expected_text)
...
def is_text_present_on_the_page(expected_text: str):
    ...
    is_text_not_present = APP.base_page.is_text_not_present(expected_text)
...
def is_modal_text_present_on_the_page(expected_text: str, action: str):
    ...
    is_text_present = APP.base_page.wait_for_text_in_temporary_message(expected_text,action)
...
def is_cohort_bar_case_count_present_on_the_page(case_count: str):
    ...
    is_case_count_present = APP.base_page.is_cohort_bar_case_count_present(case_count)
...
def is_cart_count_correct(correct_file_count: str):
    ...
    is_cart_count_correct = APP.base_page.is_cart_count_correct(correct_file_count)
...
def is_data_testid_not_present_on_the_page(data_testid: str):
    is_data_testid_present = APP.base_page.is_data_testid_present(data_testid)
...
def is_checkbox_checked(table):
    for k, v in enumerate(table):
        is_checkbox_enabeled = APP.base_page.is_facet_card_enum_checkbox_checked(v[0])
...
def is_checkbox_not_checked(table):
    for k, v in enumerate(table):
        is_checkbox_disabeled = APP.base_page.is_facet_card_enum_checkbox_checked(v[0])
...
def click_data_testid(data_testid: str):
    ...
    APP.base_page.click_data_testid(data_testid)
...
def click_button_with_data_testid(data_testid: str):
    ...
    APP.base_page.click_button_data_testid(data_testid)
...
def click_button_with_displayed_text_name(button_text_name: str):
    ...
    APP.base_page.click_button_with_displayed_text_name(button_text_name)

@step("Select the following radio buttons <table>")
def click_radio_buttons(table):
    for k, v in enumerate(table):
        APP.base_page.click_radio_button(v[0])
        time.sleep(0.1)

@step("Undo Action")
def click_undo_in_message():
    """Clicks 'undo' in a modal message"""
    APP.base_page.click_undo_in_message()
...
def select_table_value_by_row_column(table):
    ...
    """
    for k, v in enumerate(table):
        APP.base_page.select_table_by_row_column(v[0],v[1])
...
def send_text_into_search_bar(text: str, aria_label: str):
    ...
    APP.base_page.send_text_into_search_bar(text, aria_label)
...
def quick_search_and_click(text: str):
    ...
    """
    APP.base_page.quick_search_and_click(text)
