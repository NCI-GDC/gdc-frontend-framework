...
def create_save_cohort_with_random_filters(table):
    ...
    for k, v in enumerate(table):
        ...
        while (len(tabs_to_exclude) < number_of_tabs - 1) and (
            number_of_successful_filters_added < number_of_filters_to_add_in_total
        ):
            ...
            # Clicks tab in cohort builder
            APP.cohort_builder_page.click_tab_by_position_in_cohort_builder(
                random_tab_to_select
            )
            time.sleep(1)
            APP.shared.wait_for_loading_spinner_to_detatch()

            # Gets number of facet cards on the current tab
            number_of_facet_cards_on_current_tab = (
                APP.cohort_builder_page.get_number_of_facet_cards_on_current_tab_in_cohort_builder()
            )

            # random.range is inclusive at start and exclusive at stop.
            random_facet_to_select = random.randrange(
                1, number_of_facet_cards_on_current_tab + 1, 1
            )

            # Determine if facet card is able to be selected.
            # Criteria: Has checkboxes, has available data to be selected
            is_facet_able_to_be_selected = (
                APP.cohort_builder_page.is_facet_card_able_to_select_by_position(
                    random_facet_to_select
                )
            )
            if is_facet_able_to_be_selected:
                number_of_filters_on_selected_facet_card = APP.cohort_builder_page.get_number_of_filters_on_facet_card_by_position(
                    random_facet_to_select
                )
                # random.range is inclusive at start and exclusive at stop.
                random_filter_to_select = random.randrange(
                    1, number_of_filters_on_selected_facet_card + 1, 1
                )
                APP.cohort_builder_page.click_filter_by_position_on_facet_card_by_position(
                    random_facet_to_select, random_filter_to_select
                )
                APP.shared.wait_for_loading_spinner_cohort_bar_case_count_to_detatch()
                time.sleep(0.1)
                # Successfully added a filter, counter goes up
                number_of_successful_filters_added = (
                    number_of_successful_filters_added + 1
                )
