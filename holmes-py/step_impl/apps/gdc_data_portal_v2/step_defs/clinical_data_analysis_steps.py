...
def validate_default_analysis_cards_are_present(table):
    for k, v in enumerate(table):
        is_analysis_card_present = (
            APP.clinical_data_analysis.wait_until_analysis_card_is_visible(v[0])
        )
        assert is_analysis_card_present, f"The analysis card '{v[0]}' is NOT present"


@step(
    "Validate these analysis cards are not present on the Clinical Data Analysis page <table>"
)
def validate_default_analysis_cards_are_present(table):
    for k, v in enumerate(table):
        is_analysis_card_not_present = (
            APP.clinical_data_analysis.wait_until_analysis_card_is_detached(v[0])
        )
        assert (
            is_analysis_card_not_present
        ), f"The analysis card '{v[0]}' is present when it should NOT be"
...
def validate_table_value_on_analysis_card_is_present(card_name, table):
    for k, v in enumerate(table):
        ...
        assert (
            is_table_value_on_analysis_card_present
        ), f"On analysis card '{card_name}', the table does not contain the value '{v[0]}'"


@step(
    "Validate the <card_name> analysis card's table do not contain these values <table>"
)
def validate_table_value_on_analysis_card_is_present(card_name, table):
    for k, v in enumerate(table):
        is_table_value_on_analysis_card_present = (
        ...
        assert (
            is_table_value_on_analysis_card_present == False
        ), f"On analysis card '{card_name}', the table does not contain the value '{v[0]}'"


@step(
    "On the <card_name> card, select <button_name> button on the Clinical Data Analysis page"
)
...
@step(
    "Select the following values in a categorical custom bin modal on the Clinical Data Analysis page <table>"
)
...
@step(
    "Select the following hidden values in a categorical custom bin modal on the Clinical Data Analysis page <table>"
)
...
@step(
    "Name the group of values <custom_group_name> in a categorical custom bin modal on the Clinical Data Analysis page"
)
...
@step(
    "Validate <values_or_hidden_values> are <present_or_not_present> in a categorical custom bin modal on the Clinical Data Analysis page <table>"
)
def validate_value_present_or_not_present_categorical_modal(
    values_or_hidden_values, present_or_not_present, table
):
    ...
    for k, v in enumerate(table):
        ...
        if values_or_hidden_values == "values":
            is_present = APP.clinical_data_analysis.is_value_present_categorical_modal(
                v[0]
            )
        if values_or_hidden_values == "hidden values":
            is_present = (
                APP.clinical_data_analysis.is_hidden_value_present_categorical_modal(
                    v[0]
                )
            )
        ...
        if present_or_not_present == "not present":
            assert (
                not is_present
            ), f"Hidden Value '{v[0]}' is present in list when it should NOT be"


@step(
    "Select bin option <button_name> in a continuous custom bin modal on the Clinical Data Analysis page"
)
...
@step(
    "<button_name> the continuous custom bin modal on the Clinical Data Analysis page"
)
...
@step(
    "Select <button_name> in a continuous custom bin modal on the Clinical Data Analysis page"
)
...
@step(
    "Set interval of <interval_amount> with values from <min_value> to less than <max_value> in a continuous custom bin modal on the Clinical Data Analysis page"
)
def set_interval_with_min_to_max_continuous_modal(
    interval_amount, min_value, max_value
):
    """In the set interval option, set a custom interval with a min and max"""
    APP.clinical_data_analysis.set_interval_with_min_to_max_continuous_modal(
        interval_amount, min_value, max_value
    )


@step(
    "<add_or_edit> a custom range bin in a continuous custom bin modal on the Clinical Data Analysis page <table>"
)
def add_or_edit_custom_range_continuous_modal(add_or_edit, table):
    ...
    for k, v in enumerate(table):
        ...
        v[3]: row_number
        """
        APP.clinical_data_analysis.add_or_edit_custom_range_continuous_modal(
            v[0], v[1], v[2], v[3]
        )
        if add_or_edit == "add":
...
def click_delete_custom_bin_button_continuous_modal(row_number):
    ...
    APP.clinical_data_analysis.click_delete_custom_bin_button_continuous_modal(
        row_number
    )
