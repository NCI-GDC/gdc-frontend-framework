 ...
 */
const RadioButtonGroup : React.FC<RadioSquareButtonGroupProps> = ({items, checkedItem, onSelected} : RadioSquareButtonGroupProps) => {
        ...
        <form >
          <div className="inline-flex shadow-md hover:shadow-lg focus:shadow-lg" role="group">
            { items.map((x, index,  {length}) => {
              const border = length == 1 ? "rounded" : index == 0 ? "rounded-l" : index + 1 == length ? "rounded-r" : "";
              return (
