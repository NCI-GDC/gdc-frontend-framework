    ...
    <div className="flex flex-row items-center flex-nowrap border-2">
      { items.map((x,  index,  {length} ) => {
        ...
        return (
          ...
          <button
            className="hover:bg-grey text-grey-darkest font-medium px-6 rounded-sm inline-flex items-center "
           >
            { x.icon != undefined ? <Icon icon={x.icon}> </Icon> : null }
            { x.label != undefined ? <p className="px-2" >{x.label} </p>: null }
          </button>
          </div>
          </>
        )})}
...
const FromTo : React.FC <FromToProps> = ( { minimum, maximum, units= "years"} : FromToProps ) => {
...
const DaysOrYears: React.FC<FacetData> = ({ field, data, minimum=undefined, maximum=undefined }: FacetData) => {
  ...
  adjMinimum = Math.round( 10 * (adjMinimum / (units == "years"? 365.25 : 1)))/ 10;
  adjMaximum = Math.round( 10 * (adjMaximum / (units == "years"? 365.25 : 1)))/ 10;
  ...
  return (
    <div className="flex flex-col w-100 space-y-2 px-2  mt-1 ">
        <RadioButtonGroup items={["days", "years"]} checkedItem={"years"} onSelected={setUnits} />
      <div className="flex flex-row justify-between items-center">
        <input type="radio" className="form-check-input  mr-4"  id={`${field}_custom`} name={`${field}_age_range_selection`} onChange={handleRadioSelection}  />
      ...
      <FacetExpander remainingValues={remainingValues} isGroupExpanded={isGroupExpanded} onShowChanged={onShowModeChanged}/>
    </div>
  );
};

const Year: React.FC<FacetData> = ({ field, data, minimum=undefined, maximum=undefined  }: FacetData) => {
  ...
  return (
    <div className="flex flex-col w-100 space-y-2 px-2  mt-1 ">
      <div className="flex flex-row justify-between items-center">
        <input type="radio" className="form-check-input  mr-4"  id={`${field}_custom`} name={`${field}_year_range_selection`}  />
...
const Years: React.FC<FacetData> = ({ field, data, minimum=undefined, maximum=undefined    }: FacetData) => {
...
const NumericRange: React.FC<FacetData> = ({ field, data, minimum=undefined, maximum=undefined   }: FacetData) => {
  const adjMinimum = minimum != undefined ? minimum : 0;
...
const PercentRange: React.FC<FacetData> = ({ field, data, minimum=undefined, maximum=undefined   }: FacetData) => {
  ...
  return (
    <div className="flex flex-col w-100 space-y-2 px-2  mt-1 ">
      <div className="flex flex-row justify-between items-center">
        <input type="radio" className="form-check-input  mr-4"  id={`${field}_custom`} name={`${field}_percent_range_selection`}  />
        <FromTo minimum={adjMinimum} maximum={adjMaximum} units="%" />
      </div>
      <div className="flex flex-col">
        <FacetItemListHeader  items={ [ {  icon:"mdi:sort", onClick: handleSearch} ] }/>
        <div role="group" >
          {
            [...Array(bucketsToShow)].map((_, i) => {
              ...
              return (
                <div key={`${field}_${i}`} className="flex flex-row justify-start items-center form-check" >
                  <input type="radio" id={`${field}_${i}`}  name={`${field}_percent_range_selection`} className="form-radio mr-4"  /><span>From {"\u2265"} {`${low}%`} to {"<"} {`${high}%`} </span>
                </div>
              )
            })
          }
  ...
  return (
    <div>
      <div className="flex flex-col  border-2 bg-white p-1 relative drop-shadow-md border-nci-blumine-lighter">
        ...
        {{
          "age": <DaysOrYears field={field} data={data} minimum={minimum} maximum={maximum}/>,
          "year": <Year field={field} data={data}  minimum={minimum} maximum={maximum} />,
          "years": <Years field={field} data={data}  minimum={minimum} maximum={maximum} />,
          "days": <DaysOrYears field={field} data={data}  minimum={minimum} maximum={maximum} />,
