    ...
    () =>
      data?.projects
        .map((d) => {
          return {
            project: d.key,
            disease_type: [...(projectsById[d.key]?.disease_type || [])].sort(),
            primary_site: [...(projectsById[d.key]?.primary_site || [])].sort(),
            ssm_percent: data.ssmFiltered[d.key] / data.ssmTotal[d.key],
            ssm_affected_cases:
            ...
            ...(isGene && {
              cnv_gains:
                data.cnvTotal[d.key] === undefined
                  ? `0 / 0 (0.00%)`
                  : `${data.cnvGain[d.key] || 0} / ${data.cnvTotal[d.key]} (${(
                      data.cnvGain[d.key] || 0 / data.cnvTotal[d.key]
                    ).toLocaleString(undefined, {
                      style: "percent",
                      minimumFractionDigits: 2,
            ...
            ...(isGene && {
              cnv_losses:
                data.cnvTotal[d.key] === undefined
                  ? `0 / 0 (0.00%)`
                  : `${data.cnvLoss[d.key] || 0} / ${data.cnvTotal[d.key]} (${(
                      data.cnvLoss[d.key] || 0 / data.cnvTotal[d.key]
                    ).toLocaleString(undefined, {
                      style: "percent",
                      minimumFractionDigits: 2,
