...
const MutationFacetHeader: React.FC<MutationFacetProps> = ({ field, description, type = "Mutations"  } : MutationFacetProps) => {
  return (
        ...
        {data.map(({ value, count }) => {
            return (
              <div key={`${field}-${value}`} className="flex flex-row gap-x-1 px-2 text-sm ">
                ...
                <div className="flex-none text-right w-24 pr-6">{count.toLocaleString()}</div>
