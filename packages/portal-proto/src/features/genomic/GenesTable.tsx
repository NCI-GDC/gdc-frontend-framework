...
const GenesTable: React.FC<GenomicTableProps> = ({
  ...
  const { data } = useGenesTable({ pageSize: pageSize, offset: offset });
...
const GenesTableSimple: React.FC<GenesTableSimpleProps> = ({
  genes,
  filteredCases,
  cases,
  cnvCases,
      ...
      <tbody>
        {genes?.map((x) => (
          <tr key={x.id}>
            ...
            <td className="px-2">
              <Tooltip label={`Click icon to plot ${x.symbol}`}>
                <Switch
                  checked={
                    selectedSurvivalPlot
                      ? selectedSurvivalPlot.symbol == x.symbol
                      : false
                  }
                  onChange={() => handleSurvivalPlotToggled(x.symbol, x.name)}
                />
              </Tooltip>
            </td>
            <td className="px-2">
              {" "}
              {x.numCases} / {filteredCases} (
              {((x.numCases / filteredCases) * 100).toFixed(2).toLocaleString()}
              %)
            </td>
            <td className="px-2">
              {" "}
            ...
            <td className="px-2">
              {" "}
              {x.case_cnv_gain} / {cnvCases} (
              {cnvCases > 0
                ? ((x.case_cnv_gain / cnvCases) * 100)
                    .toFixed(2)
                    .toLocaleString()
                : 0.0}
              %)
            ...
            <td className="px-2">
              {" "}
              {x.case_cnv_loss} / {cnvCases} (
              {cnvCases > 0
                ? ((x.case_cnv_loss / cnvCases) * 100)
                    .toFixed(2)
                    .toLocaleString()
                : 0.0}
              %)
            ...
            <td className="px-2">
              {" "}
              {mutationCounts ? mutationCounts[x.gene_id] : " loading"}{" "}
            </td>
            <td className="px-2">
              {x.is_cancer_gene_census ? (
